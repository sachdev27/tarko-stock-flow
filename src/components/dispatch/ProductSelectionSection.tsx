...
export const ProductSelectionSection = ({
  ...
  const handleCutRoll = (entry: StockEntry) => {
    ...
    if (entry.stock_type === 'CUT_ROLL' && entry.piece_ids && entry.piece_ids.length > 0) {
      const pieceId = entry.piece_ids[0];
      // Use piece_stock_map to get the correct stock_id for this piece
      const pieceStockMap = (entry as any).piece_stock_map || {};
      const correctStockId = pieceStockMap[pieceId] || entry.stock_id;

      setSelectedStock({
        ...entry,
        stock_id: correctStockId, // Use the correct stock_id for this piece
        piece_id: pieceId, // Use the first piece_id from the array
  ...
  const handleAddCutPieces = (variant: ProductVariant, entry: StockEntry, available: number) => {
    ...
    // Include piece_stock_map so dispatch can resolve correct stock_id per piece
    const pieceStockMap = (entry as any).piece_stock_map || {};
    const roll = {
      id: entry.stock_id,
      product_variant_id: variant.id,
      piece_ids: availablePieceIds.slice(0, quantity), // Store array of piece_ids being added
      piece_stock_map: pieceStockMap, // Map of piece_id -> stock_id for dispatch
      batch_code: entry.batch_code || '',
      length_meters: entry.length_per_unit || 0,
      status: 'AVAILABLE',
      stock_type: 'CUT_ROLL',
